...
@app.post("/users/{user_id}/{event_id}/{Firstname}/{Lastname}/roster", response_model=schemas.RosterCreate)
def create_event_for_user(
    user_id: int, event_id: int, Firstname: str, Lastname: str, roster: schemas.EventCreate, db: Session = Depends(get_db)
):
    return crud.create_roster(user_id=user_id, event_id=event_id, roster=roster, db=db, Firstname=Firstname, Lastname=Lastname)
...
def set_unavailability(user_id: int, days: schemas.UnavailabilityCreate, db: Session = Depends(get_db)):
    # if user_login:
...
def get_unavailabilities(skip: int=0, limit: int = 100, db: Session = Depends(get_db)):
...
async def send_email_asynchronous():
    await send_email.send_email_async("Hello World", "tracy2anucha@gmail.com",
    "Hello World")
    return "Success"

@app.get("/send-email/backgroundtasks")
def send_email_backgroundtasks(background_tasks: send_email.BackgroundTasks):
    send_email.send_email_background(background_tasks, "Hello World", "tracy2anucha@gmail.com",
    "Hello World")
    return "Success"
...
def get_user_departments(skip: int=0, limit: int = 100, db: Session = Depends(get_db)):
...
def send_emails_by_dept(background_tasks: send_email.BackgroundTasks, dept_id: int, db: Session = Depends(get_db)):
    ...
    send_email.send_email_background(background_tasks, "Hello World",email_dept_available,"Hello World")
    return "Success"
